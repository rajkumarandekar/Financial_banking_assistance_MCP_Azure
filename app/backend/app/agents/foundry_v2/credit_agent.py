...
class CreditAgent:
    ...
    def __init__(self, azure_ai_client: FoundryChatClient, credit_mcp_server_url: str):
    ...
    async def build_af_agent(self) -> Agent:
      ...
      agent = Agent(
                ...
                instructions=CreditAgent.instructions,
                name=CreditAgent.name,
                tools=[credit_mcp_server, handoff_to_triage_agent],
                context_providers=[UserProfileProvider()]
            )
      agent.default_options["tools"] = [credit_mcp_server, handoff_to_triage_agent]
      return agent
