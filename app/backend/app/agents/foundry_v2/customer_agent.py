...
class CustomerAgent:
    ...
    def __init__(self, azure_ai_client: FoundryChatClient, customer_mcp_server_url: str):
    ...
    async def build_af_agent(self) -> Agent:
      ...
      agent = Agent(
                ...
                instructions=CustomerAgent.instructions,
                name=CustomerAgent.name,
                tools=[customer_mcp_server, handoff_to_triage_agent],
                context_providers=[UserProfileProvider()]
            )
      agent.default_options["tools"] = [customer_mcp_server, handoff_to_triage_agent]
      return agent
