...
class InvestmentAgent:
    ...
    def __init__(self, azure_ai_client: FoundryChatClient, investment_mcp_server_url: str):
    ...
    async def build_af_agent(self) -> Agent:
      ...
      investment_mcp_server = IdentityInjectingMCPTool(
                ...
                # Buying/selling moves real (demo) money and changes holdings -
                # same confirmation policy as processPayment.
                approval_mode={"always_require_approval": ["buyStock", "sellStock"]},
      )
      await investment_mcp_server.connect()
      agent = Agent(
                client=self.azure_ai_client,
                instructions=InvestmentAgent.instructions,
                name=InvestmentAgent.name,
                tools=[investment_mcp_server, handoff_to_triage_agent],
                context_providers=[UserProfileProvider()]
            )
      agent.default_options["tools"] = [investment_mcp_server, handoff_to_triage_agent]
      return agent
