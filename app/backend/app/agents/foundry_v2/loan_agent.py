...
class LoanAgent:
    ...
    def __init__(self, azure_ai_client: FoundryChatClient, loan_mcp_server_url: str):
    ...
    async def build_af_agent(self) -> Agent:
      ...
      agent = Agent(
                ...
                instructions=LoanAgent.instructions,
                name=LoanAgent.name,
                tools=[loan_mcp_server, handoff_to_triage_agent],
                context_providers=[UserProfileProvider()]
            )
      agent.default_options["tools"] = [loan_mcp_server, handoff_to_triage_agent]
      return agent
