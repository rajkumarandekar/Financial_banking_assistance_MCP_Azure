...
def create_app() -> FastAPI:
    ...
    app = FastAPI(title="Communication API and MCP server", lifespan=mcp_app.lifespan)
...
if __name__ == "__main__":
    ...
    port = 8077 if profile == "dev" else 8080
    logger.info(f"Starting communication service server with profile: {profile}, port: {port}")
