...
def create_app() -> FastAPI:
    ...
    app = FastAPI(title="Customer API and MCP server", lifespan=mcp_app.lifespan)
...
if __name__ == "__main__":
    ...
    port = 8073 if profile == "dev" else 8080
    logger.info(f"Starting customer service server with profile: {profile}, port: {port}")
