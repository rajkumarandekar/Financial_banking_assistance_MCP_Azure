...
Same cross-service pattern as payment/transaction_service_client.py and
account/transaction_service_client.py - loan repayment behavior is a real
credit-scoring input (the biggest single factor in a real FICO score), so an
EMI payment now reports itself here instead of leaving the score frozen at
its seed value forever.
"""
import logging
import os
...
async def record_credit_event(
    ...
    or block the loan payment that triggered it. Returns whether it
